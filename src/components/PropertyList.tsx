...
const PropertyList = ({ properties, isPropertyInfo = false, onOpenGallery }: PropertyListProps) => {
  ...
  const [scrollStart, setScrollStart] = useState(0);
    // State for synchronized expansion
  ...
  const LOAD_THRESHOLD = 15; // Load more when near this many properties
  const LOAD_BATCH = 15; // Load this many more properties
  const KEEP_BEFORE = 5; // Keep this many properties before current window
  
  // Calculate visible properties with lazy loading window
  const windowEnd = Math.min(windowStart + windowSize, properties.length);
  const visibleProperties = properties.slice(windowStart, windowEnd);
  ...
  // Lazy loading logic - check if we need to load more properties
  const checkLazyLoading = useCallback(() => {
    ...
    const scrollPosition = container.scrollLeft;
    const currentCardIndex = Math.floor(scrollPosition / cardWithGap);
    const visibleCardIndex = windowStart + currentCardIndex;
    
    // If we're near the end of visible properties, load more
    if (visibleCardIndex >= windowEnd - LOAD_THRESHOLD && windowEnd < properties.length) {
      const newWindowSize = Math.min(windowSize + LOAD_BATCH, properties.length - windowStart);
      setWindowSize(newWindowSize);
    }
    
    // If we're scrolled back and have room to remove from the beginning
    if (currentCardIndex < KEEP_BEFORE && windowStart > 0) {
      const removeFromStart = Math.min(KEEP_BEFORE, windowStart);
      setWindowStart(prev => Math.max(0, prev - removeFromStart));
      setWindowSize(prev => Math.min(prev + removeFromStart, properties.length - windowStart + removeFromStart));
    }
  }, [windowStart, windowEnd, windowSize, properties.length]);

  // Reset window when properties change
  useEffect(() => {
    setWindowStart(0);
    setWindowSize(20);
    setExpandedDetails(new Set());
  ...
  // Initial scroll button check when visible properties change
  useEffect(() => {
    if (scrollContainerRef.current) {
      checkScrollButtons();
    }
  ...
  return (
    <div className="relative">
      ...
      {properties.length > 1 && (
        <div className="flex items-center justify-between mb-4">          <h3 className="text-lg font-semibold text-gray-800">
            {isPropertyInfo ? 'Property Details' : `Found ${properties.length} Properties`}
            {windowEnd < properties.length && (
              <span className="text-sm text-gray-500 ml-2">
                (showing {windowStart + 1}-{windowEnd} of {properties.length})
              </span>
            )}
          </h3>
      ...
      <div className="relative overflow-hidden" data-property-list>
        <div
