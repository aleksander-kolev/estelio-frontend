...
const ChatbotDemo = () => {
  ...
  return <div className="relative bg-white rounded-xl shadow-xl overflow-hidden w-full max-w-sm mx-auto md:mx-0 border border-gray-200">
      <div className="bg-luximo-600 text-white p-4 flex flex-col items-start">
      ...
      <div className="p-4 h-96 flex flex-col overflow-y-auto bg-gray-50">
        <div className="flex-grow flex flex-col space-y-4">
          {messages.map(message => <div key={message.id} className={`flex ${message.sender === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[80%] rounded-lg p-3 ${message.sender === "user" ? "bg-luximo-600 text-white rounded-tr-none" : "bg-white border border-gray-200 text-gray-800 rounded-tl-none"}`}>
          ...
          {typingIndicator && <div className="flex justify-start">
      ...
      <div className="p-4 border-t border-gray-200 bg-white">
        <div className="flex items-center gap-2">
          <input type="text" placeholder="Напишете вашето запитване..." className="border border-gray-300 rounded-md px-3 py-2 flex-1 focus:outline-none focus:ring-2 focus:ring-luximo-500" disabled />
          <button className="bg-luximo-600 text-white p-2 rounded-md hover:bg-luximo-700" disabled>
